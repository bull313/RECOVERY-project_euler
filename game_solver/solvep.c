...
void match_regret(info_state_strategy* strategy) {
    ...
    regret_match_strat = normalize_strategy(strategy->cumulative_regret);
    ...
    for (i = 0; i < strategy->current_strategy.length; ++i) {
        list_double_set(
            &strategy->current_strategy, i, *list_double_get(regret_match_strat, i)
        );
    }

    // strategy->current_strategy = regret_match_strat;
    list_double_close(&regret_match_strat);
}
...
list_double play_decision_node(game_node* n, game_tree* g, pcontrb_stack* cs) {
    ...
    rw = (double) ( (s_iterations - 1.0) / (double) s_iterations );
    ...
    for (action = 0; action < strategy->cumulative_regret.length; ++action) {
        ...
        regret = rw * *list_double_get(strategy->cumulative_regret, action);
        regret += (1.0 - rw) * *list_double_get(action_regrets, action);
        regret = MAX( regret, 0.0 );
        
        list_double_set(&strategy->cumulative_regret, action, regret);
    }

    list_double_close(&action_utilities);
    list_double_close(&action_regrets);
    list_double_close(&immediate_utility);
